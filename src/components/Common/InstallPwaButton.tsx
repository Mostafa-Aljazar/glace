...
import { Download, Share, SquarePlus, Zap, Home, Smartphone } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function InstallPwaButton() {
  ...
  useEffect(() => {
    if (iosMode || isStandalone()) return;
  ...
  const visible = !dismissed && (iosMode || deferredPrompt !== null);

  // Order pages have their own bottom bar — keep the floating stack out of the way.
  ...
  return (
    <>
      <div className="fixed bottom-32 left-4 z-[9999998] lg:bottom-8 lg:left-8 flex flex-col items-center gap-1">
        <button
          type="button"
          onClick={() => setModalOpen(true)}
          className="flex flex-col items-center gap-1 transition-all duration-200 active:scale-90 hover:scale-110"
          aria-label="تثبيت التطبيق على الجوال"
        >
          <span className="flex justify-center items-center bg-gradient-to-br from-glace-yellow to-[#e0c93f] shadow-[0_10px_24px_rgba(0,0,0,0.28)] hover:shadow-[0_12px_32px_rgba(244,228,81,0.5)] border border-white/30 rounded-full w-[56px] h-[56px] transition-all duration-200">
            <Download size={24} strokeWidth={2.2} className="text-[#1a4a5a]" />
          </span>
          <span className="hidden sm:block font-medium text-[10px] text-white/70 leading-none">
            ثبّت التطبيق
          </span>
        </button>
      </div>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent
          showCloseButton={!iosMode}
          className="bg-[radial-gradient(circle,#41a2c5_0%,#388dab_100%)] p-0 border-0 rounded-[30px] overflow-hidden text-center text-white ring-0"
        >
          ...
          <div className="relative flex flex-col items-center px-6 sm:px-8 pt-9 pb-6 sm:pb-8">
            ...
            <div className="relative shadow-[0_14px_32px_rgba(0,0,0,0.35)] mb-4 rounded-[26%] size-24 overflow-hidden ring-2 ring-white/40">
              <Image
            ...
            <DialogHeader className="items-center gap-2">
              ...
              {!iosMode && (
                <DialogDescription className="text-[15px] text-white/85 leading-relaxed">
                  أضِف الأيقونة إلى شاشتك الرئيسية واطلب بوظتك المفضلة
                  بضغطة واحدة، بدون فتح المتصفح في كل مرة.
                </DialogDescription>
              )}
            </DialogHeader>
            ...
            {iosMode ? (
              <div className="flex flex-col gap-2.5 mt-5 w-full text-right">
                ...
                <button
                  ...
                  className="bg-white/15 hover:bg-white/25 mt-1 py-2.5 rounded-full w-full text-white text-[15px] transition-colors cursor-pointer"
                >
                  حسناً، فهمت
                </button>
              </div>
              ...
              <div className="flex flex-col gap-2.5 mt-6 w-full">
                <button
                  ...
                  className="group flex justify-center items-center gap-2 bg-glace-yellow hover:bg-white shadow-[0_10px_24px_rgba(244,228,81,0.35)] py-3 rounded-full w-full font-bold text-[#1a4a5a] text-[16px] transition-all active:scale-[0.98] cursor-pointer"
                >
                  <Download
                    size={18}
                    strokeWidth={2.4}
