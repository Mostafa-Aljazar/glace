...
const LOQAIMAT_CONFIG: {
  id: string;
  label: string;
  image: typeof luqaimat;
  ...
  image: luqaimat,
  items: [
    { label: "لقيمة عربية", price: 8, image: arabianIce },
    { label: "لقيمة لوتس", price: 8, image: lotusIce },
    { label: "لقيمة نوتيلا", price: 8, image: nutellaIce },
    { label: "لقيمة بلوبيري", price: 8, image: blueberryIce, available: false },
    { label: "لقيمة دوندورما بيستاشيو", price: 12, image: pistachioIce },
  ...
  mixes: [
    {
      ...
      options: [
        ...
        "لقيمة نوتيلا",
        "لقيمة بلوبيري",
        "لقيمة دوندورما بيستاشيو",
        "لقيمة طاقة (كل خميس)",
      ],
      optionImages: {
        "لقيمة عربية": arabianIce,
        "لقيمة لوتس": lotusIce,
        "لقيمة نوتيلا": nutellaIce,
        "لقيمة بلوبيري": blueberryIce,
        "لقيمة دوندورما بيستاشيو": pistachioIce,
        "لقيمة طاقة (كل خميس)": energyIce,
      },
    },
    {
...
export default function LoqaimatOrderClientPage() {
  ...
  function toggleMixFlavor(mixId: string, flavor: string) {
    setMixSelections((prev) => {
      ...
      const mixConfig = LOQAIMAT_CONFIG.mixes?.find((m) => m.label === mix.mixLabel);
        ...
        <div className="bg-white/17 backdrop-blur-[15px] mb-6 rounded-[28px] overflow-hidden">
          <div className="flex justify-center items-center p-8">
            <div className="flex flex-col items-center gap-6 text-center">
              ...
              <Image
                src={LOQAIMAT_CONFIG.image}
                alt={LOQAIMAT_CONFIG.label}
                width={200}
                height={200}
        ...
        <div className="bg-white/17 backdrop-blur-[15px] mb-4 rounded-[28px] overflow-hidden">
          <div className="p-5">
            ...
            <div className="flex flex-col gap-3 mb-6">
              {LOQAIMAT_CONFIG.items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!isUnavailable && (
                      <button
                        type="button"
                        onClick={() => toggleFavorite(`loqaimat-${item.label}`)}
                        className="transition-colors shrink-0"
                      >
                        <Heart
                          size={20}
                          className={
                            isFavorite(`loqaimat-${item.label}`)
            ...
            {/* Mix sections */}
            {LOQAIMAT_CONFIG.mixes && LOQAIMAT_CONFIG.mixes.length > 0 && (
              <div className="border-t border-white/15 pt-6">
                {LOQAIMAT_CONFIG.mixes.map((mixConfig) => {
                            ...
                            <>
                              ...
                              <div className="flex flex-wrap gap-3">
                                {mixConfig.options.map((flavor: string) => {
                                  ...
                                  const flavorItem = LOQAIMAT_CONFIG.items.find((item) => item.label === flavor);
