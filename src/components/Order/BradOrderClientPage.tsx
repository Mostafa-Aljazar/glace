...
// ── Main ──────────────────────────────────────────────────────────────
export default function BradOrderClientPage({
  withIceCream = false,
}: {
  ...
  function handleAddToCart() {
    ...
    if (withIceCream && !iceType)
      return showValidation("اختر نوع الأيس كريم أولاً");
    if (withIceCream && selectedFlavors.length === 0)
      return showValidation("اختر أطعمة الأيس كريم أولاً");
              ...
              <div className="flex items-end gap-2 shrink-0">
                ...
                {withIceCream && (
                  <Image
                    ...
                    width={60}
                    height={60}
                    className="drop-shadow-lg w-12 sm:w-14 h-12 sm:h-14 object-contain"
                  />
                )}
              </div>
              <div>
                ...
                <table className="w-full text-white">
                  <tbody>
                    {BRAD_SIZES.map(({ value, label, price }) => (
                      ...
                      >
                        <td className="px-2 py-2 font-medium text-[16px] text-start">
                          {label}
                        </td>
                        <td className="px-2 py-2 text-[16px] text-center">
                          {price} ₪
                        </td>
                      </tr>
                    ))}
            ...
            >
              ...
              {selectedFlavors.length > 0 && (
                <p className="mt-3 text-[12px] text-white/50">
                  {selectedFlavors.length} طعم مختار
                </p>
              )}
            </StepCard>
          )}
