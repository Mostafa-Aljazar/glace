...
export default function DessertsOrderClientPage({ initialType = "luqaimat" }: { initialType?: string }) {
  const [activeCategory, setActiveCategory] = useState(
    DESSERT_CATEGORIES.find((c) => c.id === initialType) ?? DESSERT_CATEGORIES[0]
  );
  const [selectedFlavor, setSelectedFlavor] = useState(activeCategory.items[0].label);
  const [selectedAddons, setSelectedAddons] = useState<number[]>([]);
  const [quantity, setQuantity] = useState(1);

  const addItem = useCartStore((s) => s.addItem);

  const flavorData = activeCategory.items.find((i) => i.label === selectedFlavor) ?? activeCategory.items[0];
        ...
        <div className="bg-white/[.17] backdrop-blur-[15px] rounded-[30px] w-full mb-3 overflow-hidden">
          <div className="flex md:flex-row flex-col w-full p-5">
            ...
            <div className="w-full md:w-1/2">
              <div className="bg-[#2d849e94] backdrop-blur-[20px] rounded-[20px] p-[15px_20px]">
          ...
          <div className="flex flex-col p-5">
            ...
            <div className="bg-[#2d849e94] backdrop-blur-[20px] rounded-[20px] p-5 mb-5 text-white">
              ...
              <div className="mb-5">
                ...
                <div className="flex flex-wrap gap-3">
                  {activeCategory.items.map((i) => (
                    <button key={i.label} type="button" onClick={() => setSelectedFlavor(i.label)}
                      className={`px-4 py-2 rounded-xl border text-[15px] cursor-pointer transition-colors ${
                        selectedFlavor === i.label ? "bg-white text-[#2d849e] border-white font-bold" : "bg-transparent border-white/50 text-white hover:border-white"
                      }`}>
                      {i.label}
                ...
                <div className="flex flex-wrap gap-3">
                  {ADDONS.map((addon) => (
                    <button key={addon.id} type="button" onClick={() => toggleAddon(addon.id)}
                      className={`px-4 py-2 rounded-xl border text-[15px] cursor-pointer transition-colors ${
                        selectedAddons.includes(addon.id) ? "bg-white text-[#2d849e] border-white font-bold" : "bg-transparent border-white/50 text-white hover:border-white"
                      }`}>
                      {addon.name}
                      <span className="text-[13px] mr-1">(+{addon.price} ₪)</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="flex flex-wrap justify-between items-center gap-4 px-4 text-white">
              <h3 className="text-[20px]">
                الإجمالي : <span className="font-bold text-[30px]">{((flavorData.price + addonTotal) * quantity).toFixed(2)}</span> شيكل
              </h3>
